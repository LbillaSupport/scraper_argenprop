...
def apply_scoring(records: list[dict]) -> None:
    ...
    e_lo, e_hi = _range(r.get("expenses") for r in records)

    # --- Medianas de USD/m² por zona canónica ---
    by_zone: dict[str, list[float]] = {}
    for r in records:
        zone = canonical_zone(r.get("neighborhood"))
        r["_zone"] = zone
        usd = r.get("usd_m2_covered")
        if usd is not None:
            by_zone.setdefault(zone, []).append(usd)
    ...
    for r in records:
        ...
        exp_n = _minmax(r.get("expenses"), e_lo, e_hi, invert=True)
